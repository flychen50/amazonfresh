    ...
    implements Reader
{
        ...
        throws ReaderException
    {
        ...
        int i1;
        for(i1 = l + 1; i1 < j && bitmatrix.get(i1, l); i1++);
        if(i1 == j)
            throw ReaderException.getInstance();
        int j1 = i1 - l;
        int k1;
        for(k1 = i - 1; k1 >= 0 && !bitmatrix.get(l, k1); k1--);
        if(k1 < 0)
            throw ReaderException.getInstance();
        int l1 = k1 + 1;
        if((l1 - l) % j1 != 0)
        ...
        throws ReaderException
    {
        ...
        result = new Result(decoderresult.getText(), decoderresult.getRawBytes(), aresultpoint, BarcodeFormat.DATAMATRIX);
