...
//            AbstractUPCEANWriter, EAN13Reader, AbstractUPCEANReader
...
public final class EAN13Writer extends AbstractUPCEANWriter
{
    ...
    public byte[] encode(String s)
    {
        ...
        int k = 0 + appendPattern(abyte0, 0, AbstractUPCEANReader.START_END_PATTERN, 1);
        for(int l = 1; l <= 6; l++)
        {
            int l1 = Integer.parseInt(s.substring(l, l + 1));
            if((1 & j >> 6 - l) == 1)
                l1 += 10;
            k += appendPattern(abyte0, k, AbstractUPCEANReader.L_AND_G_PATTERNS[l1], 0);
        }

        int i1 = k + appendPattern(abyte0, k, AbstractUPCEANReader.MIDDLE_PATTERN, 0);
        for(int j1 = 7; j1 <= 12; j1++)
        {
            int k1 = Integer.parseInt(s.substring(j1, j1 + 1));
            i1 += appendPattern(abyte0, i1, AbstractUPCEANReader.L_PATTERNS[k1], 1);
        }

        int _tmp = i1 + appendPattern(abyte0, i1, AbstractUPCEANReader.START_END_PATTERN, 1);
        return abyte0;
    }
