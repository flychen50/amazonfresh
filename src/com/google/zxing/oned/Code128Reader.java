                ...
                while(k1 < i) 
                {
                    ...
                    } else
                    {
                        ...
                        if(l1 == l - 1)
                        {
                            int l2 = 64;
                            int i3 = -1;
                            for(int j3 = 103; j3 <= 105; j3++)
                            {
                                int i4 = patternMatchVariance(ai, CODE_PATTERNS[j3], 179);
                                if(i4 < l2)
                                {
                                    l2 = i4;
                                    i3 = j3;
...
_L21:
_L6:
        for(; bitarray.get(l); l++);
        int i2 = Math.min(bitarray.getSize(), l + (l - k) / 2);
        if(!bitarray.isRange(l, i2, false))
            throw ReaderException.getInstance();
        if((k1 - l1 * i1) % 103 != i1)
            throw ReaderException.getInstance();
        int j2 = stringbuffer.length();
        String s;
        if(j2 > 0 && flag2)
            if(byte0 == 99)
                stringbuffer.delete(j2 - 2, j2);
            else
                stringbuffer.delete(j2 - 1, j2);
