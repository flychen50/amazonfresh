...
public final class MatrixUtil
{
        ...
        throws WriterException
    {
        ...
        if(j != bitvector.size())
            throw new WriterException("Not all bits consumed: " + j + '/' + bitvector.size());
        else
            return;
    }
        ...
        throws WriterException
    {
        ...
        int j = 0;
        while(j < bitvector.size()) 
        {
            int k = bitvector.at(bitvector.size() - 1 - j);
            bytematrix.set(TYPE_INFO_COORDINATES[j][0], TYPE_INFO_COORDINATES[j][1], k);
            if(j < 8)
                bytematrix.set(bytematrix.getWidth() - j - 1, 8, k);
            else
                bytematrix.set(8, (bytematrix.getHeight() - 7) + (j - 8), k);
            j++;
        }
    }
        ...
        throws WriterException
    {
        ...
        bitvector.appendBits(j, 5);
        bitvector.appendBits(calculateBCHCode(j, 1335), 10);
        BitVector bitvector1 = new BitVector();
        bitvector1.appendBits(21522, 15);
        bitvector.xor(bitvector1);
        ...
        throws WriterException
    {
        if(i >= 7)
        {
            ...
            while(k < 6) 
            {
                for(int l = 0; l < 3; l++)
                {
                    int i1 = bitvector.at(j);
                    j--;
                    bytematrix.set(k, l + (bytematrix.getHeight() - 11), i1);
                    bytematrix.set(l + (bytematrix.getHeight() - 11), k, i1);
                }
